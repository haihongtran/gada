    ...
    with tf.variable_scope(scope, reuse=reuse, custom_getter=getter):
        with arg_scope([leaky_relu], a=0.1), \
            ...
            layout = [
                ...
                (conv2d, (32, 3, 1), {}),
                (conv2d, (32, 3, 1), {}),
                (conv2d, (32, 5, 1), dict(padding='VALID')),
                (max_pool, (2, 2), {}),
                (dropout, (), dict(training=phase)),
                (noise, (1,), dict(phase=phase)),
                (conv2d, (64, 3, 1), {}),
                (conv2d, (64, 3, 1), {}),
                (conv2d, (64, 5, 1), dict(padding='VALID')),
                (max_pool, (2, 2), {}),
                (dropout, (), dict(training=phase)),
                (noise, (1,), dict(phase=phase)),
                (tf.reshape, ([-1, 25*64],), {}),
                (dense, (500,), dict(bn=False)),
...
def trg_generator(z):
    with tf.variable_scope('trg_gen', reuse=tf.AUTO_REUSE):
        h = bn_dense(z, 4*4*512, bias=False)    # (B,128)->(B,4*4*512)
        h = tf.reshape(h, [-1, 4, 4, 512])      # (B,4*4*512)->(B,4,4,512)
        h = bn_trans_convlayer(h, 3, 2, 256, bias=False)    # (B,4,4,512)->(B,8,8,256)
        h = bn_trans_convlayer(h, 3, 2, 128, bias=False)    # (B,8,8,256)->(B,16,16,128)
        out = trans_convlayer(h, 3, 2, 3, nonlinearity=tf.nn.tanh)  # (B,16,16,128)->(B,32,32,3)
    return out
